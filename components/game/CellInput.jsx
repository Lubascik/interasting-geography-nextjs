        ...
        <div className={styles["input-cell"]}>
            {
                ...
                <form onSubmit={(event) => { event.preventDefault() }} style={{ width: "100%", height: "auto" }}>
                    <input onKeyDown={(e)=>{if (e.key === 'Enter') {setData()}}} value={currentValue} placeholder="Column Input" onChange={(event) => { event.currentTarget.value.length < 25 ? setCurrentValue(event.currentTarget.value) : setCurrentValue(currentValue)}} className={styles["input-cell-input"]} type="text"></input>
                </form>
            }
            {
